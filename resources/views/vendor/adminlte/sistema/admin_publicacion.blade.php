...
<div class="row">
    ...
    <nav class="navbar navbar-default" style="font-size:80%;">
      <div class="container-fluid">
        ...
        <div class="collapse navbar-collapse" id="bs-example-navbar-collapse-1">
          {!! Form::open(['route'=>'adminPublicacionSearch','method'=>'get', 'class'=>'navbar-form navbar-right'])!!}
  ...
  <div class="col-md-3">
    <a href="{{route('adminPublicacionNew')}}" class="btn btn-primary btn-block margin-bottom"><i class="fa fa-plus"></i> Nueva Publicación</a>
      ...
      @foreach($fpublicaciones as $fpublicacion)
        ...
        <div class="timeline-item">
            ...
            <div class="row">
              ...
              <div class="col-sm-4">
                <div class="description-block">
                  ...
                  <h5 class="description-header">
                    <a id="p_estado{{$fpublicacion->id_fpublicacion}}" class="edit_estado btn btn-delault" data-toggle="tooltip" title="Cambiar el estado de publicación de: {{$fpublicacion->titulo}}" data-id_fpublicacion="{{$fpublicacion->id_fpublicacion}}" data-titulo="{{$fpublicacion->titulo}}">
                    {{$fpublicacion->estado}}
                    </a>
                  </h5>
                </div><!-- /.description-block -->
          ...
          <div class="timeline-footer">
            ...
            <a class="btn btn-success btn-xs" data-toggle="tooltip" title="Editar Publicación: {{$fpublicacion->titulo}}" href="{{route('adminPublicacionEdit',['id_fpublicacacion' => $fpublicacion->id_fpublicacion])}}"><i class="fa fa-edit"></i>
              Editar
            </a>
            <a class="delete-modal btn btn-danger btn-xs" data-toggle="tooltip" title="Borrar Publicación: {{$fpublicacion->titulo}}" data-id_fpublicacion="{{$fpublicacion->id_fpublicacion}}" data-titulo= "{{$fpublicacion->titulo}}"><i class="fa fa-trash"></i>
              Borrar
            </a>
          </div>
        </div>
  ...
  <div class="box-footer clearfix">
	...
	<ul class="pagination pagination-sm no-margin pull-right">
     {{$fpublicaciones->appends(array('buscar' => $old_search, 'estado' => $estado_consulta, 'importancia' => $importancia_consulta, 'tipo' => $tipo_consulta, 'tabla_consulta' => $tabla_consulta))->links()}}
	</ul>
  </div>
 </div>
...
<div id="delete_Modal" class="modal fade" tabindex="-1" role="dialog">
  <div class="modal-dialog" role="document">
      <div class="modal-content">
          <div class="modal-header">
              <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
              <h4 class="delete_title modal-title">Borrar Publicación</h4>
          </div>
          ...
          <div class="modal-body">
            <form id="form" class="form-horizontal" role="dialog">
              <div class="modal-footer">
                <button type="button" class="btn btn-default" data-dismiss="modal">Cancelar</button>
                <button type="button" class="b_delete_modal btn btn-danger">Borrar</button>
              </div>
            </form>
          </div>
        </div>
...
$(document).on('click','.edit_tipo', function(){
  ...
  $.ajax({
    ...
    type: 'post',
    url: 'adminPublicacionListaCambiarTipo',
    data: {
...
$(document).on('click', '#b_tipo_modal', function(){
  ...
  $.ajax({
    ...
    type: 'post',
    url: 'adminPublicacionUpdateTipo',
    data: {
      '_token': $('input[name=_token]').val(),
...
$(document).on('click','.edit_estado', function(){
  ...
  $.ajax({
    ...
    type: 'post',
    url: 'adminPublicacionListaCambiarEstado',
    data: {
      '_token': $('input[name=_token]').val(),
      'id_fpublicacion': $(this).data('id_fpublicacion'),
    },
...
$(document).on('click', '#b_estado_modal', function(){
  ...
  $.ajax({
    ...
    type: 'post',
    url: 'adminPublicacionUpdateEstado',
    data: {
      '_token': $('input[name=_token]').val(),
    ...
    success: function(data){
      ...
      if(data.errors){
        ...
        $('#p_estado'+data.id_fpublicacion).replaceWith("<a id='p_estado"+data.id_fpublicacion+"' class='edit_estado btn btn-delault' data-toggle='tooltip' title='Cambiar el estado de publicación : "+data.titulo+"' data-id_fpublicacion='"+data.id_fpublicacion+"' data-titulo='"+data.titulo+"'>"+data.estado+"</a>");
...
$(document).on('click','.edit_importancia', function(){
  ...
  $.ajax({
    ...
    type: 'post',
    url: 'adminPublicacionListaCambiarImportancia',
    data: {
      '_token': $('input[name=_token]').val(),
      'id_fpublicacion': $(this).data('id_fpublicacion'),
    },
...
$(document).on('click', '#b_importancia_modal', function(){
  ...
  $.ajax({
    ...
    type: 'post',
    url: 'adminPublicacionUpdateImportancia',
    data: {
      '_token': $('input[name=_token]').val(),
...
$(document).on('click','.edit_fecha', function(){
  ...
  $.ajax({
    ...
    type: 'post',
    url: 'adminPublicacionGetFpublicacion',
    data: {
      '_token': $('input[name=_token]').val(),
      'id_fpublicacion': $(this).data('id_fpublicacion'),
    },
...
$(document).on('click', '#b_fecha_modal', function(){
  ...
  $.ajax({
    ...
    type: 'post',
    url: 'adminPublicacionUpdateFecha',
    data: {
      '_token': $('input[name=_token]').val(),
...
$(document).on('click', '.b_edit_modal', function(){
  ...
  document.location.href= "{!!url('adminPublicacionEdit/"+$("#id_fpublicacion_view").val()+"');!!}";
});
