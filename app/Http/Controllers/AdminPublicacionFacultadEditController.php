...
class AdminPublicacionFacultadEditController extends Controller
{
    public function index($id_fpublicacion){

        $facultad_valida= DB::table('users')
        ->join('fpublicaciones','users.id','=','fpublicaciones.publicador')
        ->select(DB::raw('count(fpublicaciones.id_fpublicacion) AS cantidad'))
        ->where('users.id', '=', Auth()->user()->id)
        ->where('fpublicaciones.id_fpublicacion', '=', $id_fpublicacion)->get();
        ...
        $facultad = Facultad::findOrFail($fpublicacion->id_facultad);
        ...
        return view('adminlte::sistema.admin_publicacion_facultad_edit', compact('fpublicacion', 'facultad', 'tipo_publicaciones', 'importancias'));
    }
     ...
     */
    public function publicacionFacultadEdit(Request $request) {
